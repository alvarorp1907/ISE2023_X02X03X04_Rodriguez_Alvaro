...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  if (code != 0) {
	...
	//mantenemos los LEDS apagados mientras no haya eventos 
	reset_leds();
  ...
  if (len == 0) {
    ...
    //LED_SetOut (P2);
		...
		//hacemos un reseteo de todos los leds
		reset_leds();
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (strcmp (var, "led0=on") == 0) {
        P2 |= 0x01;
				 HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_0);//encendemos LED0
      }
      else if (strcmp (var, "led1=on") == 0) {
        P2 |= 0x02;
				  HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_7);//encendemos LED1
      }
      else if (strcmp (var, "led2=on") == 0) {
        P2 |= 0x04;
				  HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_14);//encendemos LED2
      }
  ...
  switch (env[0]) {
    ...
    case 'g':
      ...
      break;
			...
			case 'i':
      // AD Input from 'rtc.cgi'
      switch (env[2]) {
        case '1'://fecha y hora
           tiempo=get_time_date();
			    char buffer2[60];
          strcat(strcpy(buffer2, tiempo.time),tiempo.date);
          len = (uint32_t)sprintf (buf, &env[1], buffer2);
          break;
