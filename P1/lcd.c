...
void LCD_symbolToLocalBuffer_L1(uint8_t symbol){
  ...
  for(i=0; i<12;i++){
    ...
    buffer[i+positionL1]=value1;//pagina de arriba
    buffer[i+128+positionL1]=value2;//pagina de abajo
  }  
...
void LCD_symbolToLocalBuffer(uint8_t line,uint8_t symbol){
 ...
 void LCD_escribir_linea( char buffer_palabra1[], char buffer_palabra2[]){
	 int i,j=0;
   //limpiamos el buffer del LCD por partes
	 
	 
	 
	 for(i=0;i<strlen(buffer_palabra1);i++){
		LCD_symbolToLocalBuffer(1,buffer_palabra1[i]);
	  vacio++;
	}


	  for(j=0;j<strlen(buffer_palabra2);j++){
		LCD_symbolToLocalBuffer(2,buffer_palabra2[j]);
	  vacio++;
	}
		//CAMBIAR A THRECEPCION
	positionL1=0;	
	positionL2=256;	
